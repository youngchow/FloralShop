...
import com.zy.shop.order.domain.OrderItem;
...
public class OrderDao {
	...
	public void add(Order order) throws SQLException {
		...
		for(int i = 0; i < len; i++){
			...
			objs[i] = new Object[]{item.getOrderItemId(),item.getQuantity(),
					item.getSubtotal(),item.getFlower().getFid(),
					item.getFlower().getFname(),item.getFlower().getCurrPrice(),
					item.getFlower().getImage_b(),order.getOid()};
		}
		qr.batch(sql, objs);
	}
	 ...
	 * @throws SQLException
	 */
	public boolean updateStatus(String oid, int status) throws SQLException {
		String sql = "update t_order set status=? where oid=?";
		return qr.update(sql, status, oid) > 0 ? true : false;
	}
